...
# с очередью из 5 значений, с усреднением
# без разделения на кадры
########IMORTS#########
import rospy
import cv2
import time
...
def do_hough(frame, np_data):
    ...
    x_0 = int(((0 - y_2)*(x_1 - x_2)/(y_1 - y_2)) + x_2)
    x_4 = int(((360 - y_2)*(x_1 - x_2)/(y_1 - y_2)) + x_2)
    x_5 = x_0 + 250 # вторую линию строим с помощью вычислений
    x_6 = x_4 + 250
    cv2.line(lines_visualize, (x_0, 0), (x_4, 360), (255,0,0), 3, cv2.LINE_AA)
    cv2.line(lines_visualize, (x_5, 0), (x_6, 360), (255,0,0), 3, cv2.LINE_AA)
    box = np.array([[x_0, 0],[x_4, 360],[x_5, 0],[x_6, 360]])

    np_data = cv2.addWeighted(np_data, 0.9, lines_visualize, 1, 1)

    return np_data, canny, box
...
def msg_receiver(message):
    ...
    if (time.time() - time_last > time_to_wait) and (vehicle.mode !='LAND'):
        ...
        # im_top = np_data[0:int(y_stp/3), 0:horizontal_res]
        # im_mid = np_data[int(y_stp/3):(2*int(y_stp/3)), 0:horizontal_res]
        # im_bot = np_data[(2*int(y_stp/3)):int(y_stp), 0:horizontal_res]

        np_data, canny, box =  do_hough(np_data, np_data)
        ...
        try:
            ...
            x_bot, x_top = box[1][0], box[0][0]
            y_bot, y_top = box[1][1], box[0][1]
            ...
            ang_queue.popleft()
            ang_queue.append(angle)
            print('ang_queue: ', ang_queue)
            new_ang = np.mean(ang_queue)
            ...
            cv2.circle(np_data, (x_avg, y_stp), 10, (0,255,0), -1)
            ...
            #Очередь для усреднения значений
            err_queue.popleft()
            err_queue.append(normal_error)
            print('err_queue: ', err_queue)
            new_err = np.mean(err_queue)
            print('new_err: ', new_err)

            #ПИД-регулятор
            pid_x = PID(1, 0.1, 1,setpoint=new_err)
